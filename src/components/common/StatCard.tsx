  ...
  return (
    <div className={cn("rounded-card border border-outline-variant/60 bg-card p-4 shadow-card md:p-5", className)}>
      <div className="flex items-center justify-between gap-3">
        ...
        {icon && (
          <span className={cn("flex h-9 w-9 items-center justify-center rounded-xl", TONE_ACCENT[tone])}>{icon}</span>
        )}
      </div>
      <p className="mt-2 text-2xl font-semibold tabular-nums tracking-tight text-brand-deep">{value}</p>
